...
const PERSONA = `Ты — Пятница. Личный ассистент владельца, не чат-бот.

Женский род. На «ты». Без энтузиазма и прелюдий. Зеркалишь стиль: пишет коротко — отвечаешь коротко, матом — можешь тоже, с юмором — с юмором.

ФОРМАТ ОТВЕТОВ — главное правило:
Никакого markdown: не используй звёздочки, решётки, подчёркивания, обратные кавычки, --. Telegram их не рендерит, они торчат мусором.
Пиши чистым текстом. Списки — через тире или цифры.
Максимально коротко. Одно предложение лучше абзаца. Абзац лучше страницы.
Никаких вступлений («Конечно!», «Отличная идея!», «Понял»), никаких концовок («Надеюсь помогла», «Если что — пиши»).

КАК РАБОТАТЬ С ЗАДАЧАМИ:
Получила задачу — оцени сложность мысленно.
— Простая (до 2 часов): молча делай, отдай готовый результат. Ничего не объясняй до.
— Сложная (больше 2 часов): одна фраза заранее — сколько займёт и когда будет готово. Потом делай.
Никогда не показывай план, этапы, шаги работы — только итог.
Если не хватает данных — один конкретный вопрос. Не пять.

ПЕРЕСЛАННЫЕ СООБЩЕНИЯ ([Пересланные — N шт.]):
Изучи все разом. Выдай только: что хотят люди + что надо сделать. Без пересказа.
Если есть инструкция владельца — выполни её, используя пересланное как контекст.

ЧТО ДЕЛАТЬ АВТОМАТИЧЕСКИ:
— нарисуй / сгенерируй картинку → generate_image
— найди / загугли / что в новостях / погода → web_search (конкретная ссылка → web_fetch)
— сделай файл / документ / Word / Excel / PDF → create_document
— выпиши счёт → create_invoice (уточни только недостающие реквизиты, одной фразой; дату не спрашивай)
— напомни / поставь напоминание → set_reminder; список → list_reminders; отмени → cancel_reminder
— добавь задачу / надо сделать → add_task; список задач → list_tasks; выполнено → complete_task; удали → delete_task
— запомни / сохрани контакт / заметку → remember; покажи заметки → из памяти
— отметь энергию / я на X из 10 → log_energy; статистика → energy_log
— создай команду → create_command; удали → delete_command
— аудит / глубокий анализ / стратегия / детальный план → take_deep_task (автономная работа)
— текущие события / цены / факты которые меняются → web_search, не отвечай по памяти

ПАМЯТЬ:
Важное о владельце — сохраняй через remember (один факт, кратко). Мелочи не сохраняй.
Что знаешь — применяй, не переспрашивай.
Свои команды владельца — выполняй по инструкции когда называет.

Не знаешь — скажи прямо. Одним предложением.`;
...
// Задачи, требующие Sonnet: поиск, документы, анализ, длинные сообщения.
// Всё остальное (короткий чат, вопросы, реакции) → Haiku — быстрее и дешевле.
const SONNET_TRIGGERS =
  /найди|поищи|сделай|создай|напиши|составь|проведи|аудит|анализ|план|документ|договор|счёт|напомни|задачу|картинк|сгенерируй|погода|новости|курс|нарисуй|объясни|сравни|посмотри|расскажи|рассчитай|подбери|помоги|расставь|разложи|оцени|придумай|выпиши|реши|поставь|запомни|запиши|добавь|удали|открой|прочитай|переведи|суммаризируй|саммари|бриф|стратегия/i;

function pickModel(userText: string): string {
  if (HEAVY_TRIGGERS.test(userText)) return MODEL_HEAVY;
  const wordCount = userText.trim().split(/\s+/).length;
  if (wordCount > 15 || SONNET_TRIGGERS.test(userText)) return MODEL_DEFAULT;
  return MODEL_LIGHT; // Haiku для короткого общения без задач
}
...
const TOOLS: Anthropic.Messages.ToolUnion[] = [
  ...
  },
  {
    name: "take_deep_task",
    description:
      "Взять сложную задачу в автономную работу: провести исследование, аудит, анализ — самостоятельно, без промежуточных вопросов, и вернуться с полным готовым результатом. Вызывай когда задача требует нескольких итераций поиска и анализа. Бот немедленно пошлёт владельцу подтверждение и запустит работу в фоне.",
    input_schema: {
      type: "object",
      properties: {
        confirmation_message: {
          type: "string",
          description:
            "Сообщение владельцу: что берёшь в работу и что будешь делать. 2-3 строки, включи оценку времени.",
        },
        task_prompt: {
          type: "string",
          description:
            "Самодостаточный подробный промпт для выполнения задачи. Включи все данные (URL, требования, контекст) — это будет отдельный запуск без доступа к текущему диалогу.",
        },
      },
...
export type FridayCallbacks = {
  ...
  onDeepTask?: (confirmationMessage: string, taskPrompt: string) => Promise<void>;
};
...
async function executeTool(
  ...
  if (block.name === "take_deep_task") {
    const input = block.input as {
      confirmation_message?: string;
      task_prompt?: string;
    };
    if (!input.confirmation_message?.trim() || !input.task_prompt?.trim()) {
      return "Нужны сообщение-подтверждение и промпт задачи.";
    }
    if (callbacks.onDeepTask) {
      await callbacks.onDeepTask(
        input.confirmation_message.trim(),
        input.task_prompt.trim(),
      );
    }
    return "Задача принята и запущена в фоне. Результат отправится владельцу отдельным сообщением.";
  }
...
const DEEP_TASK_INSTRUCTION = `РЕЖИМ АВТОНОМНОЙ ЗАДАЧИ:
Работаешь самостоятельно — без промежуточных уточнений. Используй web_search и web_fetch столько, сколько нужно для полного анализа.

Этапы работы:
1. Сбор данных (ищи, читай страницы, изучай).
2. Анализ: сильные стороны, слабые стороны, что мешает.
3. Конкретные рекомендации с приоритетами.
4. Готовый промпт для Claude — скопируй-вставь, чтобы сразу запустить следующий шаг.

Структура итогового ответа (без markdown-символов, только текст и тире):
Резюме: [3-5 строк о главном]
...
[полный самодостаточный промпт, который владелец вставит в Claude Code или Claude.ai и сразу получит результат]`;
...
export type RunFridayOptions = {
  ...
  briefing?: boolean;
  /** Дополнительная инструкция поверх персоны (напр. для business-режима). */
  systemExtra?: string;
  /** ID владельца для загрузки фактов/команд когда chatId = чужой чат (business). */
  ownerChatId?: number;
  /** Режим глубокой задачи: Opus, 30 итераций, DEEP_TASK_INSTRUCTION. */
  deepTask?: boolean;
};
...
export async function runFriday(
  ...
  // Deep-задача всегда на Opus; иначе выбираем по тексту или опции.
  const model = options.deepTask
    ? MODEL_HEAVY
    : (options.model ?? pickModel(userText));
  // Болтовня вроде «привет» / «спасибо» инструментов не требует — экономим
  // токены. Брифинги и deep-задачи всегда с инструментами.
  const useTools = options.briefing || options.deepTask || !isChitchat(userText);
  const tools = useTools ? cachedTools() : undefined;
  // Deep-задача может занять много шагов (поиск, анализ, синтез).
  const maxSteps = options.deepTask ? 30 : 10;

  // Долговременная память + свои команды владельца — в контекст разговора.
  // В business-режиме chatId = чужой чат, поэтому факты грузим по ownerChatId.
  const memId = options.ownerChatId ?? chatId;
  const facts = await getFacts(memId);
  const commands = await listCommands(memId);
  const contextParts: string[] = [];
  if (options.deepTask) contextParts.push(DEEP_TASK_INSTRUCTION);
  if (options.systemExtra) contextParts.push(options.systemExtra);
  if (facts.length > 0) {
    contextParts.push(
  ...
  return options.deepTask
    ? "Задача оказалась слишком масштабной — попробуй разбить её на части и задать каждую отдельно."
    : "Я закопалась в задаче — давай попробуем ещё раз, чуть проще.";
}
